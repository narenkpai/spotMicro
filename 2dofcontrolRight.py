...
while True:
    for i in range(totalPoints+1):
        ...
        if k == 1:
            set_servo_angle(6, (90 -((180-angleval)/2)) + subAngle)
            set_servo_angle(9, 180 - angleval)
            k = 2
            break
        if k == 2:
            set_servo_angle(0,(90 +((180-angleval)/2)) - subAngle)
            set_servo_angle(1, angleval)
            k = 3
            break
        if k == 3:
            set_servo_angle(7, (90 -((180-angleval)/2)) + subAngle )
            set_servo_angle(8, 180- angleval)
            k = 4
            break
        if k == 4:
            set_servo_angle(2,(90 +((180-angleval)/2)) - subAngle )
...
while(True):
    for i in range(20,220):
        angleval = solve_angle_c(130,130,i)
        print(angleval)
        set_servo_angle(6,(90-((180-angleval)/2)) + 20)
        set_servo_angle(7,(90-((180-angleval)/2)) + 20)
        set_servo_angle(8,180 - angleval)
        set_servo_angle(9,180 - angleval)
        set_servo_angle(10,180)
        set_servo_angle(11,0)
